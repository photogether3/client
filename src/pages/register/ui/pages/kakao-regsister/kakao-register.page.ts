...
import { AuthApi, KakaoAuthService, PoliciesDTO, TokenService } from 'src/entities/auth';
import { ButtonComponent } from 'src/shared/components';
import { FooterWidget } from 'src/widgets/footer';
import { HeaderWidget } from 'src/widgets/header';

import { PolicyDetailComponent } from '../01.policy-select/detail';

@Component({
  selector: 'kakao-register-page',
  templateUrl: './kakao-register.page.html',
  imports: [FooterWidget, HeaderWidget, ButtonComponent],
  host: {
    class: 'h-screen',
  },
})
export class KakaoRegisterPage {
  /** -------------------------------------------------------
   * PRIVATE PROPERTIES
   * -------------------------------------------------------*/
  ...
  onNext() {
    const { id, email } = this.kakaoAuthService.getProvider();
    ...
    const dto = {
      ...
      providerId: id,
      providerEmail: email,
      policyIds: this.policyAgreedId(),
    };

    this.kakaoAuthService.registerKakao(dto).subscribe({
      next: (res) => {
